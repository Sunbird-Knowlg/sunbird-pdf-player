...
export class TelemetryService {
  ...
  private context: any; private config: any; private onTelemetryEvent?: (event: any) => void;
  ...
  public initialize({ context, config, metadata }: PlayerConfig, onTelemetryEvent?: (event: any) => void) {
    this.context = context; this.config = config; this.onTelemetryEvent = onTelemetryEvent; this.playSessionId = this.uniqueId();
    if (!context) return;
    if (!$t.isInitialized) {
      const telemetryConfig: Partial<TelemetryConfig> = {
        pdata: context.pdata || { id: 'in.sunbird', ver: '1.0' }, env: 'contentplayer',
      ...
      telemetryConfig.dispatcher = {
        dispatch: (event: any) => {
          if (context.dispatcher) context.dispatcher.dispatch(event);
